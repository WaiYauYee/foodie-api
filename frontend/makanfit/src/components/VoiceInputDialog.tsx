...
export const VoiceInputDialog: React.FC<VoiceInputDialogProps> = ({
  ...
  const handleExecuteCommand = () => {
    ...
    if (detectionResult.intent === 'LOG_WATER' && detectionResult.waterAmountMl) {
      onAddWater(detectionResult.waterAmountMl);
      confetti({
        particleCount: 25,
        spread: 50,
        origin: { y: 0.8 },
      ...
      setLastLoggedMessage(`Added ${detectionResult.waterAmountMl} ml water`);
      setTimeout(() => {
        onClose();
      }, 1100);
    } else if (detectionResult.intent === 'LOG_FOOD' && detectionResult.matchedFood) {
      ...
      const finalFood: Food = {
        ...food,
        servingSize: detectionResult.unit === 'g' ? factor : Math.round(food.servingSize * factor),
        nutrients: {
          ...food.nutrients,
      ...
      const catName = targetCat === 'snack' ? 'Snacks' : targetCat.charAt(0).toUpperCase() + targetCat.slice(1);
      ...
      <div className="fixed inset-x-0 bottom-0 bg-white z-[130] rounded-t-[32px] p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 max-w-md mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between pb-2 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">Auto-detects food, portion & meal</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col items-center justify-center space-y-3 py-1">
              ...
              <div className="text-center">
                ...
                <p className="text-[11px] text-slate-400">
                  e.g. <em>"Ate Nasi Lemak for breakfast"</em> or <em>"Drank 2 glasses of water"</em>
                </p>
              </div>
            </div>
            ...
            {detectionResult.canExecute && (
              <div className="bg-white rounded-2xl p-4 border border-emerald-200 shadow-xs space-y-3 animate-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    {detectionResult.intent === 'LOG_FOOD' && (
                      <MealCategoryIcon type={detectionResult.category || defaultCategory} size={48} />
                    )}
                    ...
                    <div>
                      ...
                      <h4 className="font-black text-slate-900 text-base leading-tight">
                        {detectionResult.intent === 'LOG_WATER'
                          ? `+${detectionResult.waterAmountMl} ml Water`
                  ...
                  <div className="text-right">
                    <p className="text-base font-black text-emerald-600">
                      {detectionResult.intent === 'LOG_WATER'
                        ? `${detectionResult.waterAmountMl} ml`
                        : `${Math.round(
                            (detectionResult.matchedFood?.nutrients.calories || 0) * (detectionResult.quantity || 1)
                          )} Cal`}
                    </p>
                ...
                {detectionResult.canExecute && detectionResult.intent === 'LOG_FOOD' && onOpenManualEdit && (
                  <button
                    onClick={() => {
                      stopListening();
                      const targetCat = detectionResult.category || defaultCategory;
                      onOpenManualEdit(detectionResult.matchedFood!, targetCat);
                      onClose();
                    }}
