...
export const Diary: React.FC<DiaryProps> = ({
  ...
  const totalWaterMl = todaysWaterEntries.reduce(
  ...
  const [voiceTargetCategory, setVoiceTargetCategory] =
    useState<MealType>("breakfast");

  const handleOpenVoiceForCategory = (cat: MealType) => {
    setVoiceTargetCategory(cat);
    setIsVoiceInputOpen(true);
  };

  const [manualEditFromVoice, setManualEditFromVoice] = useState(false);
  const [resumeVoiceSession, setResumeVoiceSession] = useState(false);
  ...
  const [servingSizeMode, setServingSizeMode] = useState<"serving" | "g">(
    "serving",
  );
  const [servingSizeValue, setServingSizeValue] = useState(1);
  const [gramsValue, setGramsValue] = useState(0);
  const [isDeleteIngredientModalOpen, setIsDeleteIngredientModalOpen] =
    useState(false);
  ...
  const searchResults = useMemo(() => {
    ...
    return SEARCHABLE_FOODS.filter(
      (f) =>
  ...
  useEffect(() => {
    if (servingSizeMode === "serving" && selectedFood) {
      ...
      const calculatedGrams =
        servingSizeValue * (selectedFood.servingSize || 250);
  ...
  const todaysMeals = meals.filter(
    (m) => new Date(m.createdAt).toDateString() === selectedDate.toDateString(),
  );
  ...
  const handleCapture = async (base64: string) => {
    ...
    try {
      ...
      if (segmentationResult.segmentation_id) {
        ...
        calorieResult = await getCaloriesBySegmentation(
          segmentationResult.segmentation_id,
        );
      ...
      const totalCalories =
        calorieResult.success && calorieResult.total_nutrition
      ...
      setSelectedFood({
        ...
        nutrients: {
          ...
          calories: Math.max(
            0,
            Math.round(
              (n.calories - (removed?.nutrients?.calories || 0)) * 10,
            ) / 10,
          ),
          protein: n.protein || 0,
          carbs: n.carbs || 0,
    ...
    if (selectedFood) {
      ...
      setSelectedFood({
        ...
        nutrients: {
          ...
          calories:
            Math.round((selectedFood.nutrients.calories + calorieDiff) * 10) /
            10,
        },
      });
    }
    ...
    if (editingMealId) {
      ...
      if (existingMeal) {
        onUpdateMeal({
          ...
          ingredients: ingredients.length > 0 ? ingredients : [updatedFood],
  ...
  const handleAddIngredientFinal = () => {
    ...
    const adjustedCalories = Math.round(
      tempIngredient.nutrients.calories * tempServingValue,
    );
    const newIng: Food = {
    ...
    const newDishCalories =
      Math.round((selectedFood.nutrients.calories + adjustedCalories) * 10) /
      10;
    setSelectedFood({
      ...selectedFood,
      nutrients: {
    ...
    <div className="bg-[#F8FAFC] min-h-screen">
      ...
      <div className="bg-white px-4 py-4 shadow-sm sticky top-0 z-[70]">
        ...
        {isCalendarOpen && (
          <>
            <div
              className="fixed inset-0 bg-black/40 z-[89] backdrop-blur-sm"
            ...
            <div className="fixed inset-x-0 bottom-0 bg-white z-[90] rounded-tl-3xl rounded-tr-3xl p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 max-w-md mx-auto">
              ...
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-black text-gray-800 uppercase tracking-normal">
                  Select Date
                </h3>
                <button
                  onClick={() => setIsCalendarOpen(false)}
        ...
        <div className="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 space-y-6">
          <div className="flex justify-between items-start text-center">
            <div className="space-y-1">
              ...
              <p className="font-black text-gray-900 text-xl">
                {targetCal}{" "}
                <span className="text-xs text-gray-400 font-normal">kcal</span>
              </p>
            </div>
          ...
          <div className="relative h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min((totals.calories / targetCal) * 100, 100)}%`,
              }}
            />
          ...
          <p className="text-[11px] font-black uppercase text-gray-400 tracking-widest text-center">
            Macronutrients
          </p>
          ...
          {mealCategories.map((cat) => {
            ...
            const categoryCalories = Math.round(
              categoryMeals.reduce(
                (sum, m) => sum + (m.food.nutrients?.calories || 0),
                0,
              ),
            );
            const displayName =
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <div>
                      <h3 className="text-[#1A2A33] font-black uppercase tracking-widest text-xs">
                        {displayName}
                      </h3>
                ...
                {categoryMeals.length > 0 ? (
                  ...
                  <p className="pt-1 text-xs text-slate-400 font-medium">
                    No foods logged yet. Tap to view or add{" "}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <p className="font-black text-gray-900 text-lg">
                {waterL}{" "}
                <span className="text-xs text-gray-400 font-normal">L</span>
              </p>
              {/* <button
      ...
      >
        <SlidersHorizontal size={16} className="stroke-[2.5]" />
          ...
          <div className="relative h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min((totalWaterMl / WATER_GOAL_ML) * 100, 100)}%`,
              }}
            />
          </div>
        ...
        {/* VOICE INPUT MODAL (ZERO API CALLS) */}
        <VoiceInputDialog
          isOpen={isVoiceInputOpen}
          resumeSession={resumeVoiceSession}
          onClose={() => {
            setIsVoiceInputOpen(false);
            setResumeVoiceSession(false);
          }}
          defaultCategory={voiceTargetCategory}
          onAddMeal={(food, cat) => onAddMeal(food, cat)}
          onAddWater={(ml) => addWaterEntry(ml)}
          onOpenManualEdit={(food, cat) => {
            setActiveCategory(cat);
            setSelectedFood(food);
            setIngredients(food.ingredients || []);
            setGramsValue(food.servingSize || 250);
            setIsSingleFoodEditOpen(true);
            setManualEditFromVoice(true);
          }}
        />
                ...
                <div className="grid grid-cols-4 gap-2">
                  {WATER_PRESETS.map((preset) => (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-bold text-sky-700 leading-tight text-center">
                        {preset.label}
                      </span>
        ...
        {/* ADD MENU CHOICE (AI Scan vs Search vs Voice) */}
        {addMenuCategory && (
          <>
            <div
            ...
            <div className="fixed inset-x-0 bottom-0 bg-white z-[120] rounded-t-[32px] p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 max-w-md mx-auto space-y-4">
              ...
              <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                <h3 className="text-lg font-black text-slate-900 tracking-normal">
                  Add to{" "}
                  {addMenuCategory === "snack"
                    ? "Snacks"
                    : addMenuCategory.charAt(0).toUpperCase() +
                      addMenuCategory.slice(1)}
                </h3>
                <button
                  onClick={() => setAddMenuCategory(null)}
                  className="p-1 text-gray-400 hover:text-gray-600 rounded-full"
              ...
              <div className="grid grid-cols-3 gap-2.5 pt-2">
                ...
                <button
                  onClick={() => {
                    const cat = addMenuCategory!;
                    setAddMenuCategory(null);
                    handleOpenVoiceForCategory(cat);
                  }}
                  className="p-3.5 rounded-2xl bg-gray-50 hover:bg-gray-100 border border-gray-100 flex flex-col items-center text-center gap-2 cursor-pointer transition-all active:scale-98"
                >
                  <div className="w-11 h-11 rounded-2xl bg-[#1E293B] text-white flex items-center justify-center shadow-xs">
                    <Mic size={19} />
                  </div>
                  <div>
                    <p className="text-[#1A2A33] font-bold text-xs">
                      Voice Log
                    </p>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400">Food camera</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400">Malaysian DB</p>
                  </div>
                </button>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto pb-10">
            ...
            <div className="px-6 py-8 text-center space-y-4">
              <div className="flex items-center justify-center space-x-2">
                <h2 className="text-4xl font-black text-[#1A2A33]">
                  {activeCategory}
                </h2>
              </div>
              <div className="space-y-1.5">
                <p className="text-lg font-bold text-gray-800">
                  {Math.round(categoryCurrent.calories)} / {targetCal} Cal
                </p>
                <div className="w-full max-w-[240px] mx-auto h-2.5 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-green-500 rounded-full transition-all duration-1000"
                    style={{
                      width: `${Math.min((categoryCurrent.calories / targetCal) * 100, 100)}%`,
                    }}
                  />
                </div>
                ...
                <div className="w-full max-w-[240px] mx-auto flex justify-between text-[11px] font-bold text-slate-400">
                  <span>
                    {Math.round((categoryCurrent.calories / targetCal) * 100)}%
                    of target
                  </span>
                  ...
                  <span>
                    {Math.max(
                      0,
                      targetCal - Math.round(categoryCurrent.calories),
                    )}{" "}
                    Cal remaining
                  </span>
                </div>
            ...
            <div className="grid grid-cols-4 gap-2 px-6 mb-10">
              <MacroRing
                label="Protein"
                current={categoryCurrent.protein}
                target={categoryTargets.protein}
                color="#FF9999"
              />
              <MacroRing
                label="Fat"
                current={categoryCurrent.fat}
                target={categoryTargets.fat}
                color="#FFE066"
              />
              <MacroRing
                label="Carbs"
                current={categoryCurrent.carbs}
                target={categoryTargets.carbs}
                color="#99E0FF"
              />
              <MacroRing
                label="Fiber"
                current={categoryCurrent.fiber}
                target={categoryTargets.fiber}
                color="#D5B4B4"
              />
            </div>
                ...
                <div className="bg-white rounded-[28px] p-8 border border-slate-100 text-center space-y-4 shadow-xs">
                  ...
                  <div>
                    <p className="font-bold text-slate-800 text-base">
                      No foods logged for {activeDisplayName}
                    </p>
                    <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
                      Snap a photo or search our Malaysian food library to log
                      your meal.
                    </p>
                  </div>
                  <button
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs font-semibold text-slate-400 mt-0.5">
                            {meal.food.servingSize || 100}g •{" "}
                            {meal.food.nutrients.calories} Cal
                          </p>
                        </div>
                        ...
                        <div className="bg-slate-50 rounded-xl p-2.5 text-xs text-slate-600 font-medium space-y-1">
                          ...
                          <p className="leading-relaxed">
                            {meal.ingredients
                              .map(
                                (i) =>
                                  `${i.name} (${i.nutrients?.calories || 0} Cal)`,
                              )
                              .join(", ")}
                          </p>
                        </div>
              ...
              <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Total nutrition for all foods in {activeDisplayName}
                  </p>
                </div>
              </div>
              <div className="border-[4px] border-black p-5 space-y-4 text-black max-w-sm mx-auto shadow-sm">
                <h2 className="text-3xl font-black border-b-[10px] border-black pb-1 uppercase italic leading-none tracking-tighter">
                  Nutrition Facts
                </h2>
                ...
                <div className="space-y-2">
                  {[
                    {
                      name: "Total Fat",
                      value: currentNutrients?.fat,
                      perc: Math.round(
                        ((currentNutrients?.fat || 0) / 65) * 100,
                      ),
                    },
                    {
                      name: "Total Carbohydrate",
                      value: currentNutrients?.carbs,
                      perc: Math.round(
                        ((currentNutrients?.carbs || 0) / 300) * 100,
                      ),
                    },
                    {
                      name: "Protein",
                      value: currentNutrients?.protein,
                      perc: Math.round(
                        ((currentNutrients?.protein || 0) / 50) * 100,
                      ),
                    },
                  ].map((item, i) => (
                    <div
            ...
            <div className="absolute top-0 left-0 w-full p-6 flex items-center justify-between z-10">
              <button
                onClick={() => {
                  ...
                  setSelectedFood(null);
                  setEditingMealId(null);
                  if (manualEditFromVoice) {
                    setManualEditFromVoice(false);
                    setResumeVoiceSession(true);
                    setIsVoiceInputOpen(true); // reopen voice dialog
                  }
                }}
                  ...
                  <div className="absolute right-0 mt-0 w-64 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="bg-sky-50 p-4 rounded-2xl border border-sky-100 shadow-lg">
                      <p className="text-[10px] text-sky-800 font-bold leading-relaxed">
                        Can't recognize the dish? Our AI handles 120 Malaysian
                        local foods, but if it's not covered, Search Manually
                        instead!
                      </p>
                      <p className="text-[10px] text-sky-800 font-bold leading-relaxed">
                        Segmentation might sometimes be off, partially wrong, or
                        fail to detect ingredients if the image is unclear.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-50">
              <div className="flex items-center space-x-3">
                {(() => {
                  const sentiment = getSentiment(
                    currentNutrients?.calories || 0,
                  );
                  return (
                    <>
                      <div
                        className={`w-10 h-10 ${sentiment.bg} rounded-full flex items-center justify-center`}
                      >
                        <sentiment.Icon
                          className={`w-6 h-6 ${sentiment.color}`}
                        />
                      </div>
              ...
              {calorieData?.calorie_range_kcal ? (
                <div className="text-xl font-black text-gray-800">
                  ...
                  {Math.round(calorieData.calorie_range_kcal.max_kcal)}{" "}
            ...
            <div className="px-6 py-0 pt-0">
              <div className="relative">
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-6 h-6 text-gray-400 transition-transform ${isServingSizeOpen ? "rotate-180" : ""}`}
                  />
                </button>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-2xl shadow-lg z-50 overflow-hidden">
                    <button
                      onClick={() => {
                        ...
                        const newServing =
                          gramsValue / selectedFood?.servingSize;
                    ...
                    >
                      <span className="text-[15px]">
                        {parseFloat(
                          (gramsValue / selectedFood?.servingSize).toFixed(2),
                        )}{" "}
                        serving ({Math.round(gramsValue)} g)
                      </span>
                      {servingSizeMode === "serving" && (
                    ...
                    >
                      <span className="text-[15px]">
                        {Math.round(
                          servingSizeValue * selectedFood?.servingSize,
                        )}{" "}
                        g ({servingSizeValue.toFixed(1)} serving)
                      </span>
              ...
              {activeResultTab === "nutrition" ? (
                <div className="space-y-8">
                  ...
                  <div className="flex-1 overflow-y-auto px-6 py-8 space-y-6 pt-0">
                    ...
                    <div className="border-[3px] border-black p-4 space-y-4 text-black">
                      <h2 className="text-2xl font-black border-b-8 border-black pb-1 uppercase italic leading-none">
                        Nutrition Facts
                      </h2>
                        ...
                        <div className="flex justify-between items-baseline border-b-4 border-black pb-1">
                          ...
                          {calorieData?.calorie_range_kcal ? (
                            <span className="text-lg font-bold">
                              {Math.round(
                                calorieData.calorie_range_kcal.min_kcal,
                              )}
                              ...
                              {Math.round(
                                calorieData.calorie_range_kcal.max_kcal,
                              )}{" "}
                              kcal
                            </span>
                          ) : (
                        ...
                        {currentNutrients &&
                          [
                            ...
                            },
                            {
                              ...
                              perc: Math.round(
                                ((currentNutrients?.fiber ?? 0) / 25) * 100,
                              ),
                            },
                            {
                              name: "Protein",
                ...
                <div className="space-y-6">
                  ...
                  <p className="text-[10px] text-center text-gray-400 font-medium px-4">
                    ...
                    AI-estimated food composition based on detected ingredients.
                    Results may be inaccurate and should be manually reviewed.
                  </p>
                  ...
                  <div className="space-y-4">
                    {ingredients.map((ing, idx) => (
                      ...
                      >
                        <button
                          onClick={() =>
                            setExpandedIngredientIdx(
                              expandedIngredientIdx === idx ? null : idx,
                            )
                          }
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            <div
                              className={`transition-transform duration-200 ${expandedIngredientIdx === idx ? "rotate-90" : ""}`}
                            >
                              <ChevronRight
                                size={18}
                              ...
                              <div className="grid grid-cols-4 gap-1.5 mb-3">
                                <div className="text-center p-1.5 bg-white rounded shadow-sm">
                                  ...
                                  <p className="text-xs font-bold text-purple-700">
                                    {(ing.nutrients.protein || 0).toFixed(1)}g
                                  </p>
                                </div>
                                <div className="text-center p-1.5 bg-white rounded shadow-sm">
                                  <p className="text-[7px] font-black text-gray-400 uppercase">
                                  ...
                                  <p className="text-xs font-bold text-sky-700">
                                    {(ing.nutrients.carbs || 0).toFixed(1)}g
                                  </p>
                                </div>
                                <div className="text-center p-1.5 bg-white rounded shadow-sm">
                                  <p className="text-[7px] font-black text-gray-400 uppercase">
                                  ...
                                  <p className="text-xs font-bold text-orange-700">
                                    {(ing.nutrients.fat || 0).toFixed(1)}g
                                  </p>
                                </div>
                                <div className="text-center p-1.5 bg-white rounded shadow-sm">
                                  <p className="text-[7px] font-black text-gray-400 uppercase">
                                  ...
                                  <p className="text-xs font-bold text-amber-700">
                                    {(ing.nutrients.fiber || 0).toFixed(1)}g
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center space-x-2">
                                <div className="flex-1 h-1.5 bg-white/60 rounded-full overflow-hidden">
                                  <div
                                    ...
                                    style={{
                                      width: `${Math.min((ing.nutrients.calories / (selectedFood?.nutrients.calories || 1)) * 100, 100)}%`,
                                    }}
                                  />
                                </div>
                                <span className="text-[9px] font-bold text-gray-500 w-8 text-right">
                                  {Math.round(
                                    (ing.nutrients.calories /
                                      (selectedFood?.nutrients.calories || 1)) *
                                      100,
                                  )}
                                  %
                                </span>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="relative">
                <button
                  onClick={() =>
                    setIsTempServingSizeOpen(!isTempServingSizeOpen)
                  }
                ...
                >
                  ...
                  <ChevronDown
                    className={`text-gray-400 w-6 h-6 transition-transform ${isTempServingSizeOpen ? "rotate-180" : ""}`}
                  />
                </button>
                ...
                {isTempServingSizeOpen && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl z-[160] overflow-hidden">
                    ...
                    >
                      <span>
                        {tempServingValue.toFixed(1)} serving (
                        {Math.round(tempServingValue * tempBaseGrams)} g)
                      </span>
                      {tempServingSizeMode === "serving" && (
                        <Check className="w-5 h-5" />
                      )}
                    ...
                    >
                      <span>
                        {Math.round(tempGramsValue)} g (
                        {tempServingValue.toFixed(1)} serving)
                      </span>
                      {tempServingSizeMode === "g" && (
                        <Check className="w-5 h-5" />
                      )}
                    </button>
                  </div>
            ...
            <div className="flex flex-col items-center text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-gray-400 font-bold leading-relaxed">
                  ...
                  <span className="text-gray-600">"{selectedFood?.name}"</span>{" "}
                  from your diary? This action cannot be undone.
                </p>
              </div>
      ...
      {isShowingNutritionLabel && selectedFood && (
        <div className="fixed inset-0 bg-white z-[120] flex flex-col animate-in fade-in duration-300">
          <div className="p-4 flex items-center justify-center border-b border-gray-100 relative">
            <button
              onClick={() => setIsShowingNutritionLabel(false)}
              className="absolute left-4 p-2 hover:bg-gray-100 rounded-full text-gray-400 transition-colors"
            >
              <ChevronLeft size={24} />
            </button>
            <div className="flex items-center space-x-2">
            ...
            <div className="border-[3px] border-black p-4 space-y-4 text-black">
              ...
              <div className="border-b border-black py-1">
                <div className="flex justify-between items-baseline font-bold">
                  <span className="text-sm">Serving size</span>
                  <span className="text-lg">
                    {selectedFood.servingSize || 458}g
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                {[
                  {
                    name: "Total Fat",
                    value: selectedFood.nutrients.fat,
                    perc: Math.round((selectedFood.nutrients.fat / 65) * 100),
                  },
                  {
                    name: "Total Carbohydrate",
                    value: selectedFood.nutrients.carbs,
                    perc: Math.round(
                      (selectedFood.nutrients.carbs / 300) * 100,
                    ),
                  },
                  {
                    name: "Protein",
                    value: selectedFood.nutrients.protein,
                    perc: Math.round(
                      (selectedFood.nutrients.protein / 50) * 100,
                    ),
                  },
                ].map((item, i) => (
                  <div
                  ...
                  >
                    <p className="text-sm font-bold">
                      {item.name}{" "}
                      <span className="font-normal">{item.value}g</span>
                    </p>
                    <span className="text-sm font-black">{item.perc}%</span>
                  </div>
                ))}
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-400 font-medium">
                    {f.group || "Malaysian Dish"} • {f.servingSize}g
                  </p>
                </div>
                <div className="text-right">
